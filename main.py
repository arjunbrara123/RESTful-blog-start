...
def add():
    new_post = BlogPost()
    for attr in new_post.args:
        new_post.__setattr__(attr, new_post.args.get(attr))
...
@app.route('/')
def get_all_posts():
    return render_template("index.html", all_posts=refresh_db())
    ...
    if request.method == "POST":
        ...
        for key, value in form.data.items():
            if hasattr(new_post, key):
                new_post.__setattr__(key, value)
